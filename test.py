...
class TestVersion11Parsing(unittest.TestCase):
    expected_results = {
        'Per base sequence quality': PASS_RESULT,
        ...
        'Overrepresented sequences': WARN_RESULT,
        'Adapter Content': PASS_RESULT,
        'Kmer Content': FAIL_RESULT,
    }

    def setUp(self):
        self.file = "examples/rna_v11.txt"
        self.parser = FastQCParser(filename = self.file)

    def test_version(self):
        self.assertEqual(self.parser.version, '0.11.2')
    ...
    def test_basic_statistics(self):
        ...
        self.assertEqual(table[0]['Measure'], 'Filename')
        self.assertEqual(table[0]['Value'], 'DS28704B_ATGTCA_L002_R1_001.fastq.gz')
        ...
        self.assertEqual(table[3]['Measure'], 'Total Sequences')
        self.assertEqual(table[3]['Value'], '13608491')

        self.assertEqual(table[4]['Measure'], 'Sequences flagged as poor quality')
        self.assertEqual(table[4]['Value'], '1048299')

        self.assertEqual(table[5]['Measure'], 'Sequence length')
        self.assertEqual(table[5]['Value'], '76')
    ...
    def test_total_overrepresented_sequences(self):
        ...
        self.assertEqual(self.parser.get_total_percent_overrepresented_sequences(), Decimal('6.32713796114499407'))

    def test_info_values(self):

        info_values = self.parser.get_module_info_values("Sequence Duplication Levels")

        self.assertEqual(info_values['Total Deduplicated Percentage'], '42.86031810408766')
